					...
					for (int i = 0; i < TAM_RESERVA; i += 14)
					{
						if (reservas[i] == codigoReserva)
						{
							if (resposta == 1)
							{
								for (int poltrona = i + 2; poltrona < i + 8; poltrona++)
								{
									if (reservas[poltrona] != 0 && assentosDestinosNacionais[voo][reservas[poltrona] + 4] != 2)
									{
										voo = reservas[i + 1];
		...
		case 7:;
			...
			for (int i = 0; i < TAM_RESERVA; i += 14)
			{
				if (reservas[i] == codigoReserva)
				{
					...
					else if (assentosDestinosNacionais[reservas[i + 1]][reservas[i + 2] + 4] == 0) printf(" - RESERVA CANCELADA -\n");
					system("pause");
					break;
				}
			}
