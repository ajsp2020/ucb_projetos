...
int main()
{
	...
	char destinos[QTD_VOOS][40] = { "Rio Branco", "Maceio", "Macapa", "Manaus", "Salvador", "Fortaleza", "Brasilia", "Vitoria", "Goiania", "Sao Luis",
	...
	int qtdAssentosPorVoo[QTD_VOOS][QTD_ASSENTOS];
	for (int i = 0; i < QTD_VOOS; i++)
	{
		for (int j = 0; j < QTD_ASSENTOS; j++) qtdAssentosPorVoo[i][j] = 0;
	}
	for (int i = 0; i < QTD_VOOS; i++) qtdAssentosPorVoo[i][0] = i;
	
	int ponteiro = 0; // Variavel que controla o ponteiro da lista de reservas
	...
	do {// INCIO DO WHILE MENU:
		...
		case 2:;
			...
			do {
				...
				for (int voo = 0; voo < QTD_VOOS; voo++)
				{
					...
					if (strncmp(destinos[voo], destino, 5) == 0)
					{
						/* 2.1 -> inicio: pega a quantidade de assento: */
						if (qtdAssentosPorVoo[voo][1] != 0)
						{
						...
						do {
							...
							printf("   ==> ");
							status = scanf("%d", &qtdAssentosPorVoo[voo][1]);

							respostaValida = qtdAssentosPorVoo[voo][1] >= 90 && qtdAssentosPorVoo[voo][1] <= 200 && status == 1;
						...
						do {
							...
							qtdAssentosPorVoo[voo][2] = precoReal * 100;
		...
		case 3:; 
			...
			for (int i = 0; i < QTD_VOOS; i++)
			{
				...
				if (strncmp(destinos[i], destino, 5) == 0)
				{
					voo = i;
					temDestino = 1;
				}
			...
			if (temDestino == 1 && qtdAssentosPorVoo[voo][1] != 0)
			{
				system("cls");
				printf(" - DESTINO: %s\n", destinos[voo]);
				...
				if (fazerReserva == 1)
				{
					...
					do {
						...
						{
							reservas[ponteiro] = reserva; // O primeiro vai receber a reserva
							numeroVoo = ponteiro + 1;
				...
				/* 3.2.2 -> Fim: Cria um código de reserva único e adiciona na lista de reservas seu numero e voo correspondente */
					...
					do { 
						...
						do {
							...
							printf(" - DESTINO: %s\n", destinos[voo]);
							printf(" - TOTAL DE ASSENTOS: %d\n", qtdAssentosPorVoo[voo][1]); // colocar o assento disponivel menos oq ja foi escolhido
							...
							printf("\n\t\t\t\t\t\t   a b c        d e f\n");
							for (int i = 0; i < qtdAssentosPorVoo[voo][1]; i++)
							{
								...
								if (qtdAssentosPorVoo[voo][i + 4] == 0)
								{
									printf("\033[0;32m");
									printf("D ");
									printf("\033[0;37m");
								}
								else if (qtdAssentosPorVoo[voo][i + 4] == 1)
								{
									printf("\033[0;31m");
									printf("R ");
									printf("\033[0;37m");
								...
								else if (qtdAssentosPorVoo[voo][i + 4] == 2)
								{
									printf("\033[0;34m");
									printf("C ");
									printf("\033[0;37m");
					...
					if (idade <= 5) preco = qtdAssentosPorVoo[voo][2] * 0.5;
					else preco = qtdAssentosPorVoo[voo][2];
					...
					if (qtdAssentosPorVoo[voo][posicao + 4] != 0)
					{
					...
					else
					{
						...
						{
							qtdAssentosPorVoo[voo][posicao + 4] = 1;
			...
			else
			{
				...
				else if (qtdAssentosPorVoo[voo][1] == 0) printf(" - VOO NÃO CADASTRADO! - \n");
				system("pause");
			}
					...
					if (resposta == 1) 
					{
						for (int poltrona = i + 2; poltrona < i + 8; poltrona++)
						{
							if (reservas[poltrona] != 0)
							{
								...
								voo = reservas[i + 1];
								qtdAssentosPorVoo[voo][3] += somaInteira; // <<< Arrumar a sona que está imprimindo em dados do voo com o valor errado e arrumar melhor o código
								qtdAssentosPorVoo[voo][reservas[poltrona] + 4] = 2;
							}
						}
					}
				}
					...
					for (int i = 0; i < TAM_RESERVA; i += 14)
					{
						if (reservas[i] == codigoReserva)
						{
							if (resposta == 1)
							{
								for (int poltrona = i + 2; poltrona < i + 8; poltrona++)
								{
									if (reservas[poltrona] != 0 && qtdAssentosPorVoo[voo][reservas[poltrona] + 4] != 2)
									{
										voo = reservas[i + 1];
										qtdAssentosPorVoo[voo][reservas[poltrona] + 4] = 0;
										reservas[poltrona] = 0;
									}
								}
							}
		...
		case 6:
			...
			for (int poltrona = 0; poltrona < qtdAssentosPorVoo[voo][1]; poltrona++)
			{
				if (qtdAssentosPorVoo[voo][poltrona + 4] == 0) assentosDisponiveis++;
				else if (qtdAssentosPorVoo[voo][poltrona + 4] == 1) assentosReservados++;
				else if (qtdAssentosPorVoo[voo][poltrona + 4] == 2) assentosConfirmados++;
			}
			printf("- TOTAL DE ASSENTOS DISPONÍVEIS: %d\n", assentosDisponiveis);
			printf("- TOTAL DE ASSENTOS RESERVADOS: %d\n", assentosReservados);
			printf("- TOTAL DE ASSENTOS CONFIRMADOS: %d\n", assentosConfirmados);
			printf("\n- VALOR TOTAL VENDIDO: R$ %.2f", (float)qtdAssentosPorVoo[voo][3] / 100);
		...
		case 7:;
			...
			for (int i = 0; i < TAM_RESERVA; i += 14)
			{
				if (reservas[i] == codigoReserva)
				{
					...
					printf(" - TAXA DE EMBARQUE: R$ 35,00\n");
					printf(" - PREÇO TOTAL: R$ %.2f\n\n", soma + 35.00);
					if (qtdAssentosPorVoo[reservas[i + 1]][reservas[i + 2] + 4] == 1) printf(" - RESERVA NÃO CONFIRMADA - \n");
					else if (qtdAssentosPorVoo[reservas[i + 1]][reservas[i + 2] + 4] == 2) printf(" - RESERVA CONFIRMADA -\n");
					else if (qtdAssentosPorVoo[reservas[i + 1]][reservas[i + 2] + 4] == 0) printf(" - RESERVA CANCELADA -\n");
					system("pause");
					break;
				}
			}
